...
import { senddogeNative , Bitcoin_estimatedGas } from '../utils/function';
...
const ConfirmDogeTransaction = ({route, navigation}) => {
  ...
  const confirmAndSend = async () => {
    ...
    console.log("sdasdasdasaaaaa")
    try {
      setLoader(true)
      let response = await senddogeNative(trxDetail?.privateKey, trxDetail?.recipientAddress, trxDetail?.amount );
  ...
  let fee = 1
  let validity = Number(trxDetail?.amount) + fee
      ...
      <View style={[styles.confirmAmountWrapperFlex ,{marginTop:50}]}>
        <View style={styles.confrimAmountCenterWrapper}>
          ...
          <View style={styles.confirmAmountFlex}>
            ...
            <Text style={[styles.swapConfirmValue, {color: theme.text}]}>
            {(trxDetail?.balanceAmount - fee)?.toFixed(6)}
            </Text>
            {/* <Text style={[ {color: theme.text}]}>
              {trxDetail?.symbol}
            </Text> */}
      ...
      <View
        ...
        <View style={styles.gasFeeFlex}>
          ...
          <View>
            ...
            {Number(trxDetail?.amount) + Number(fee)} DOGE
...
export default ConfirmDogeTransaction;
